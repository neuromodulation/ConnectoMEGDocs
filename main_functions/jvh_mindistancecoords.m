function [coord_closest, index, distance] = jvh_mindistancecoords(coord_interest,coord_target, tolerance)
...
arguments (Input)
    ...
    coord_target   (:,3) double % coord_targets = coorddouble =  jvh_coordcell2double(cellofchannels) = an atlas or collection of MNI coordinates;
...
amount_of_targets   = size(coord_target, 1);
...
for ii=1:amount_of_interests
    ...
    distances(abs(coord_interest(ii,1)-coord_target(:,1))>=tolerance)=tolerance/2;
    distances(abs(coord_interest(ii,2)-coord_target(:,2))>=tolerance)=tolerance/2;
    distances(abs(coord_interest(ii,3)-coord_target(:,3))>=tolerance)=tolerance/2;
    for i = 1:amount_of_targets
        if distances(i)<tolerance/2
            distances(i)=pdist([coord_target(i,:);coord_interest(ii,:)],'euclidean');
        end
    end
    
    if min(distances)==tolerance/2
        [coord_closest(ii,:),index(ii,:), distance(ii,:)] = jvh_mindistancecoords(coord_interest(ii,:),coord_target, tolerance*2);
    else
        [distance(ii,:),index(ii,:)] = min(distances);
        coord_closest(ii,:) = coord_target(index(ii,:),:);
    end
end
end
